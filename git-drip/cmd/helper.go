...
func gitDir() string {
	return trim(cmdOutput("git", "rev-parse", "--git-dir"))
}
...
func origin(config map[string]string) string {
	if hasKey(config, "gitdrip.origin") {
		return config["gitdrip.origin"]
	}
	return "origin"
}

func requireDripInit() {
	if !isDripInitialized() {
